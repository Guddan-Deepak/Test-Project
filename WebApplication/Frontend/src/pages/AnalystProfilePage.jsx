...
import { Camera, Mail, Phone, User, Shield, CheckCircle, XCircle, Key, Save, Edit2, Briefcase } from 'lucide-react';
...
const AnalystProfilePage = () => {
    ...
    const fetchProfile = async () => {
        try {
            const { data } = await api.get('/analyst/profile');
            if (data.success) {
                setProfile(data.data);
                setFormData({
                    name: data.data.name,
                    priorityEmail: data.data.email,
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            const { data } = await api.put('/analyst/profile/image', uploadData, {
    ...
    const handleUpdateProfile = async (e) => {
        e.preventDefault();
        try {
            const { data } = await api.put('/analyst/profile', {
                name: formData.name,
                email: formData.priorityEmail,
    ...
    const handleChangePassword = async (e) => {
        ...
        try {
            const { data } = await api.put('/analyst/profile/password', {
    ...
    const handleResendVerification = async () => {
        try {
            ...
            const response = await api.post('/analyst/profile/resend-verification');
            console.log("Response received:", response);
            const { data } = response;
            console.log("Response Data:", data);

            if (data.success) {
                console.log("Success condition met, showing toast");
                toast.success("Verification link sent! Check your email.");
            } else {
                console.warn("Success condition NOT met", data);
            }
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-[#050B14] p-8 text-slate-200 font-sans selection:bg-cyan-500/30">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                {/* Header Section */}
                <div>
                    <h1 className="text-xl font-medium text-cyan-400">My Profile</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-[#0B1221] p-6 rounded-3xl border border-cyan-500/30 shadow-[0_0_30px_rgba(6,182,212,0.15)] flex flex-col">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className={`group flex items-center bg-[#080E1A] border rounded-xl transition-all overflow-hidden ${isEditing ? 'border-cyan-500/30 focus-within:border-cyan-500 focus-within:ring-1 focus-within:ring-cyan-500/20' : 'border-slate-800/50'}`}>
                                            ...
                                            <input
                                                ...
                                                disabled={!isEditing}
                                                className="w-full bg-transparent py-3.5 pr-4 text-sm text-slate-200 focus:outline-none disabled:text-slate-400"
                                                value={formData.priorityEmail}
                                                onChange={(e) => setFormData({ ...formData, priorityEmail: e.target.value })}
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className={`group flex items-center bg-[#080E1A] border rounded-xl transition-all overflow-hidden ${isEditing ? 'border-cyan-500/30 focus-within:border-cyan-500 focus-within:ring-1 focus-within:ring-cyan-500/20' : 'border-slate-800/50'}`}>
                                            ...
                                            <input
                                                type="tel"
                                                disabled={!isEditing}
                                                className="w-full bg-transparent py-2.5 pr-4 text-sm text-slate-200 focus:outline-none disabled:text-slate-400"
                        ...
                        {/* Specialization Card */}
                        <div className="bg-[#0B1221] p-8 rounded-3xl border border-cyan-500/30 shadow-[0_0_30px_rgba(6,182,212,0.15)]">
                            <div className="flex items-center gap-4 mb-6">
                                <div className="p-3 bg-cyan-500/10 rounded-2xl">
                                    <Briefcase size={24} className="text-cyan-400" />
                                </div>
                                <div>
                                    <h3 className="text-lg font-bold text-white">Specialization</h3>
                                    <p className="text-sm text-slate-500">Your area of expertise</p>
                                </div>
                            </div>

                            <div className="p-4 bg-[#080E1A]/50 border border-slate-800/50 rounded-2xl flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <Briefcase size={18} className="text-cyan-400" />
                                    <span className="text-slate-200 font-medium">{profile.specialization || "General"}</span>
                                </div>
                                <span className="px-3 py-1 rounded-lg bg-cyan-500/10 text-cyan-400 text-xs font-bold uppercase tracking-wider border border-cyan-500/20">Active</span>
                            </div>
                        </div>
